...
var _extends2;
return _extends({}, x, (_extends2 = {}, _extends2['data-hoofd'] = '1', _extends2));
}),
scripts: scripts,
...
} : (_ref = {}, _ref[meta.keyword] = meta[meta.keyword], _ref.content = escape(meta.content), _ref);
})
};
}
};
...
if (options.integrity) _script.setAttribute('integrity', options.integrity);
if (options.crossorigin) _script.setAttribute('crossorigin', options.crossorigin);
if (options.defer) _script.setAttribute('defer', 'true');else if (options.async) _script.setAttribute('async', 'true');
...
var memoizedMetas = useMemo(function () {
...
return x.keyword === keyword && x.name === meta.name && x.charset === meta.charset && x['http-equiv'] === meta.httpEquiv && x.property === meta.property && x.content === meta.content;
});
...
return x.keyword === meta.keyword && meta[meta.keyword] === x[meta.keyword];
});
...
dispatcher._change(TITLE, prevTitle.current, prevTitle.current = title);
} else {
...
return function () {
hasMounted.current = false;
if (prevTitle.current != null) dispatcher._removeFromQueue(TITLE, prevTitle.current);
};
}, []);
useEffect(function () {
if (language) document.getElementsByTagName('html')[0].setAttribute('lang', language);
}, [language]);
};
...
export { HoofdProvider, createDispatcher, toStatic, useHead, useLang, useLink, useMeta, useScript, useTitle, useTitleTemplate };
